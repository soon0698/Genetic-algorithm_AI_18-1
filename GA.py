...
class Chromosome:
    ...
    def __init__(self, gene):
        ...
        self.invidual_fitness = Chromosome.get_invidual_fitness(gene)
    ...
    @staticmethod            
    def get_invidual_fitness(gene):
        """
        Helper method used to return the fitness for the chromosome based
        on its gene.
        """
        fitness = np.zeros(len(Chromosome._target_gene))
        for i in range(len(Chromosome._target_gene)):
            fitness[i] = abs(ord(gene[i]) - ord(Chromosome._target_gene[i]))
        return fitness
